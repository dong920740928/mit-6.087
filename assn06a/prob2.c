...
int add_word(const char* word, char* translation)
{
    ...
    // find or create node for word
    int word_len = strlen(word);
    int new_node_count = 0;
    struct s_trie_node* p = proot;
    ...
    if (p->translation == NULL) {
        char* translation_str = (char*)malloc(sizeof(char) * LINE_MAX);
        strcpy(translation_str, translation);
        p->translation = translation_str;

    } else {
        int old_len = strlen(p->translation);
        p->translation[old_len] = ',';
        strcpy(p->translation + old_len + 1, translation);
    }
