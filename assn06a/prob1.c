...
struct token_queue infix_to_postfix(struct token_queue* pqueue_infix)
{
    ...
    p_expr_token next_token = NULL;
    p_expr_token prev_token = NULL;

    queue_postfix.front = NULL;
    queue_postfix.back = NULL;

    while ((next_token = dequeue(pqueue_infix))) {
        switch (next_token->type) {
        case OPERAND:
            enqueue(&queue_postfix, next_token);
            break;
        case OPERATOR:
            /* push all the operators which precedences higher than next token into postfix queue */
            while ((prev_token = pop(&operator_stack))) {
                if (prev_token->type == OPERATOR && op_precedences[prev_token->value.op_code] >= op_precedences[next_token->value.op_code]) {
                    enqueue(&queue_postfix, prev_token);
                } else {
                    push(&operator_stack, prev_token);
                    break;
                }
            }
            push(&operator_stack, next_token);
            break;
#if PARSE_PARENS
        case LPARENS:
            push(&operator_stack, next_token);
            break;
        case RPARENS:
            /* push all the operators between parenthese into postfix queue */
            while ((prev_token = pop(&operator_stack))) {
                if (prev_token->type == OPERATOR) {
                    enqueue(&queue_postfix, prev_token);
                } else {
                    // parenthese
                    free(prev_token);
    ...
    /* push all remain operators info postfix queue */
    while ((prev_token = pop(&operator_stack))) {
        enqueue(&queue_postfix, prev_token);
    }
...
double evaluate_postfix(struct token_queue* pqueue_postfix)
{
    ...
    p_expr_token operand_stack = NULL;
    p_expr_token next_token = NULL;

    while ((next_token = dequeue(pqueue_postfix))) {
        if (next_token->type == OPERAND) {
            push(&operand_stack, next_token);
        } else if (next_token->type == OPERATOR) {
            /* find operands for the operator */
            int operands_num = op_operands[next_token->value.op_code];
            p_expr_token operands[operands_num];
            double result;
            ...
            switch (next_token->value.op_code) {
            case ADD:
                result = operands[0]->value.operand + operands[1]->value.operand;
                break;
            case SUBTRACT:
                result = operands[0]->value.operand - operands[1]->value.operand;
                break;
            ...
            case DIVIDE:
                ...
                break;
            case NEGATE:
                result = -operands[0]->value.operand;
            }

            /* push result into operands stack */
            enum token_type type = OPERAND;
            union token_value value;
            value.operand = result;
            p_expr_token result_token = new_token(type, value);
    ...
    /* the only remaining operands is the final result */
