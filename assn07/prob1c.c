...
int find_index(nodekey key, p_tnode pnode)
{
    ...
    while (L <= R) {
        ...
        if (icmp == 0) {
            ...
            if (L <= M - 1) {
                // reduce search range to left
                R = M - 1;
            } else {
                // key is less than any item in search range
                ibetween = M;
                break;
            }
        } else {
            if (R >= M + 1) {
                // reduce search range to right
                L = M + 1;
            } else {
                // key is greater than any item in search range
                ibetween = M + 1;
                break;
            }
        }
...
nodevalue* find_value(const nodekey key)
{
    ...
    while (pleaf != NULL) {
        ichild = find_index(key, pleaf);
        if (ichild < 0) {
            // find key at -ichild - 1
            return pleaf->values[-ichild - 1];
        }

        // try to find at child node
        pleaf = pleaf->children[ichild];
    }
...
void get_query(char* buffer)
{
    fgets(buffer, MAX_QUERY_LEN, stdin);
}

int pick_newline(char* str)
{
    int len = 0;
    while (str[len] != '\n') {
        ++len;
...
int main(int argc, char* argv[])
{
    ...
    if (ret) {
        fprintf(stderr, "Error: Can not open database: %s\n", sqlite3_errmsg(db));
        sqlite3_close(db);
    ...
    ret = sqlite3_exec(db, sql, store_result, NULL, &exec_err);
    if (ret != SQLITE_OK) {
        fprintf(stderr, "SQL error: %s\n", exec_err);
        sqlite3_free(exec_err);
    }

    for (get_query(key_query); pick_newline(key_query) > 0; get_query(key_query)) {
        nodevalue* val = find_value(key_query);
        if (val) {
