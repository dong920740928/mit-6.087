...
void initialize_world_from_file(const char * filename) {
    ...
    if (file == NULL){
        perror("initialize world error");
        abort();
    }

    char buffer[MAX_CELL_PER_LINE];
    int next_line = 0;
    while (fgets(buffer, MAX_CELL_PER_LINE, file) != NULL){
        int len = strlen(buffer);
        len = len > WORLDWIDTH ? WORLDWIDTH : len;
        for (int i = 0; i < len; ++i){
            world[i][next_line] = buffer[i] == '*' ? ALIVE : DEAD;
        }
        for (int i = len; i < WORLDWIDTH; ++i){
            world[i][next_line] = DEAD;
        }

        ++next_line;
        if (next_line == WORLDHEIGHT){
            break;
        }
    }

    for (; next_line < WORLDHEIGHT; ++next_line){
...
void save_world_to_file(const char * filename) {
    ...
    if (file == NULL){
        perror("save world error");
        abort();
    }

    char buffer[WORLDWIDTH+ 1];
    for (int i = 0; i < WORLDHEIGHT; ++i){
        for (int j = 0; j < WORLDWIDTH; ++j){
            buffer[j] = world[j][i] == ALIVE ? '*' : ' ';
        }
        buffer[WORLDWIDTH] = '\0';
